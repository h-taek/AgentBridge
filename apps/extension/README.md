# AgentBridge — VS Code 익스텐션

> VS Code / Antigravity IDE 익스텐션. 컨셉·동작 원리·CLI 요구사항은 [모노레포 README](https://github.com/h-taek/AgentBridge/blob/main/README.md) 참고.
...
양쪽 앱 공통 기능(메모리 패널·refine 정책·hook 자동 주입 등)은 [모노레포 README](https://github.com/h-taek/AgentBridge/blob/main/README.md) 참고.

## 설치

VS Marketplace에서 *AgentBridge* 검색 후 설치. (Antigravity IDE도 동일하게 사이드 메뉴에서 익스텐션 설치 가능.)
...
- **AgentBridge 메타데이터(IR · turns · archive · sessions)** — VS Code `globalStorage` 격리 경로 안의 워크스페이스별 디렉토리
- **CLI hook 파일** — 사용자 워크스페이스 `cwd/`에 마커 블록 merge 방식으로 추가:
  - `.codex/hooks.json` + `.codex/config.toml`
  - `.agents/hooks.json`
  - claude는 globalStorage 격리 settings.json만 사용, cwd 무침범

## 라이선스

MIT — 자세한 내용은 [LICENSE](https://github.com/h-taek/AgentBridge/blob/main/LICENSE) 참고.

모노레포: https://github.com/h-taek/AgentBridge

...
- 장기 메모리 — 프로젝트를 넘어 오래 쓰이는 지식(역할·관례·작업하는 repo와 도메인)을 프로필로 모은다. 대화에서 기억해둘 만한 것을 자동으로 제안하고, 새 "장기 메모리" 사이드바 뷰에서 승인/버림하면 승인한 메모를 이후 관련 프롬프트에 자동으로 끼워준다. 프로필 폴더를 열어 평문 마크다운으로 직접 편집할 수도 있다.

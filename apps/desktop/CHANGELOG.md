...
- Codex sessions now handle Codex's new collapsible hook-context display. AgentBridge's injected context appears as only a marker line in Codex's collapsed preview, so its content no longer shows on screen, and the older display filter that could occasionally cut terminal output short has been removed.

...
- Apple Silicon 전용 — 이제 AgentBridge는 Apple Silicon(arm64) Mac용으로만 배포된다. Intel Mac은 더 이상 지원하지 않는다.
...
- 백그라운드 사용량 측정 격리 강화 — agy·codex 백그라운드 사용량(쿼터) 측정이 항상 격리된 환경에서만 실행된다. 격리 환경 준비에 실패하는 드문 경우엔 비격리로 돌리지 않고 그 회차를 건너뛰어, 백그라운드 측정이 실제 CLI 설정 디렉토리에 잔재 파일을 남기지 않는다.

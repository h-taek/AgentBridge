// agentbridge-memory.js 단일 소스 → 두 앱 resources/bin로 번들 동기화 (V-16 + §G3).
//
// 원본: packages/core/bin/agentbridge-memory.js (번들 엔트리)
// 산출: 두 앱의 resources/bin/ — esbuild로 core 검색을 인라인한 self-contained CJS.
//   호스트가 `node <resources/bin/...>`로 별도 프로세스 spawn하므로 실제 파일이 각 앱에 있어야 한다.
//
...
// 각 앱 빌드(desktop build / extension compile) 직전에 실행된다.
...
console.log(`[sync-helper] agentbridge-memory.js bundled → ${dests.length} app(s)`);

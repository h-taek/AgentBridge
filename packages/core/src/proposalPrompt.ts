...
const CATEGORY_GUIDE = [
  ...
  '- repos — the user\'s ongoing project portfolio, only when they treat a project as a long-term, recurring part of their work — NOT a description of whatever app this single session happens to be about',
  '- domain — domain expertise the USER carries between projects — NOT this app\'s features, product spec, or what it does',
...
const DISCRIMINATOR = [
  ...
  'A candidate must pass BOTH tests:',
  '1. Time/transfer: "Would this still be useful next month, to a *different* agent in a *different* repo?"',
  '   If it is only true *inside this codebase* or *right now*, it FAILS.',
  '2. User vs project: "Is this about HOW the user works / who they are, or about WHAT this specific app is?"',
  '   Keep the first. Drop the second — even if it is stable and long-lived.',
  '',
  ...
  '5. The identity, purpose, features, or domain of the specific app/project in this session — e.g. "the user is building a specific app such as a mod manager or a tool for X", or "the app has a tabbed settings screen". These describe the project, not the user. Even phrased durably, drop them.',
  '',
...
export function buildProposalPrompt(args: ProposalPromptArgs): string {
  ...
  return [
    ...
    '',
    'Focus on the USER — who they are and HOW they work — not on WHAT this particular project is.',
    'This conversation happens to be about one project; abstract away from it. A good fact would still',
    'read correctly inside a completely different project. If a candidate only makes sense because you',
    'know which app this session is about, it is the wrong kind of fact — drop it.',
    '',
    CATEGORY_GUIDE,
    '',
    DISCRIMINATOR,
    ...
    '2. Each element: { "category": <one of the 7>, "title": string, "summary": string (1 line), "body": string (1-3 sentences), "confidence": number 0..1, "indexEntries": string[] }.',

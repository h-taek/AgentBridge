// buildCompactionPrompt — turns.jsonl의 oldest N record + 직전 IR → 슬림화된 IR JSON 요청 prompt.
//
// 본문은 영어로 작성. LLM 일관성·재현성을 위해 prompt language는 English로 통일.
// LANGUAGE_RULE에서 IR 본문 자체는 사용자가 사용한 언어를 따라가도록 명시.
